...
proc freq data = ind;
	tables train;
	title 'Training and Validation Breakdown';
run;
...
proc reg data = ind plots =  diagnostics(unpack) outest = Model_Final_est;
...
* plot final model MAE by year sold;
proc sgplot data=Model_Final_validation;
	title 'Error by Year Sold';
	vbox err / category=YrSold;
	where train = 0;
run; quit;
