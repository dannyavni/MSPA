...
proc reg data = keep plots =  diagnostics(unpack);
  ...
  output out=outlier (keep=GrLivArea AgeAtSale LotArea SalePrice r lev dfitt) h=lev dffits=dfitt rstudent=r;
...
proc sgscatter data=outlier2 datasymbols=(CircleFilled);
   plot SalePrice*LotArea / group=outlier_def transparency=0.5 markerattrs=(size=8);
   title 'Scatter Plot of Sale Price to Lot Area by Outlier Code';
run;
...
proc sgplot data=keep;
  title 'Histogram of Living Area with Outlier';
  histogram GrLivArea / transparency=0.5;
  density GrLivArea / type=normal;
  density GrLivArea / type=kernel;
run; quit;

proc sgplot data=sans_outlier;
  title 'Histogram of Living Area without Outlier';
  histogram GrLivArea / transparency=0.5;
  density GrLivArea / type=normal;
  density GrLivArea / type=kernel;
run; quit;
...
proc sgplot data=keep;
  title 'Histogram of Building Age with Outlier';
  histogram AgeAtSale / transparency=0.5;
  density AgeAtSale / type=normal;
  density AgeAtSale / type=kernel;
run; quit;

proc sgplot data=sans_outlier;
  title 'Histogram of Building Age without Outlier';
  histogram AgeAtSale / transparency=0.5;
  density AgeAtSale / type=normal;
  density AgeAtSale / type=kernel;
run; quit;
...
data outlier3;
	...
	if (GrLivArea > 2000) then do;
	  outlier_def = '1. Living Area Over 2000';
	  outlier_code = 1;
	end;
	else if (AgeAtSale > 100) then do;
	  outlier_def = '2. Building Age over 100';
	  outlier_code = 1;
	end;
	else if (AgeAtSale < 2) then do;
	  outlier_def = '3. Building Age Under 5';
	  outlier_code = 1;
	end;
    else if (LotArea > 12000) then do;
	  outlier_def = '4. Lot Area Over 12000';
	  outlier_code = 1;
	end;
	else do;
	  outlier_def = '5. Regular Observation';
...
proc freq data = outlier3;
  tables outlier_def;
  title 'Sample Waterfall for Outlier Detection';
run; quit;

* model 4;
proc reg data = outlier3 plots =  diagnostics(unpack);
...
proc print data = ind(obs=10);
  var SalePrice GrLivArea AgeAtSale new_bldg old_bldg;
run; quit;
...
proc reg data = ind plots =  diagnostics(unpack);
  model logSalePrice = GrLivArea LotArea AgeAtSale
	...
	quality_index  brick_exterior lot_frontage new_bldg old_bldg pos_cond
  / vif;
...
proc sgscatter data=keep;
    compare x=(AgeAtSale GrLivArea) y=logSalePrice / transparency=0.5 reg loess;
	title 'Scatter Plot of Sale Price by Age, Living Area, Lot Area, And Basement Area';
run; quit;
